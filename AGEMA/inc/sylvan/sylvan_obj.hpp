...
public:
    ...
    /**
     * @brief Compute the logarithmic number of satisfying variable assignments, using the given number of variables.
     */
    double SatCountLn(const size_t nvars) const;
...
public:
	...
	/**
	 * @brief Extract the Bdd from this set.
	 */
	Bdd extract() const {
		return set;
	}
	
	int equals(const BddSet &other) const {
		return (set == other.set);
	}
	
	int leq(const BddSet &other) const {
		return (set <= other.set);
	}
